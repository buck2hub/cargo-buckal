...
representation with a proper DAG (directed acyclic graph) built on the `daggy` crate.
...
cargo metadata ──> BuckalResolve::from_metadata()
   ...
   │   resolve: BuckalResolve  <── single DAG  │
   │   workspace_root                          │
...
The DAG correctly models diamond dependencies where two intermediate crates depend on different
...
unifies them into a single node, and the DAG reflects this correctly — both intermediate crates
